  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        id: tripDoc.id,
        ...tripDoc.data(),
        createdAt: tripDoc.data().createdAt?.toDate?.()?.toISOString() || new Date().toISOString(),
...
export function useCreateTrip() {
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["/api/trips", variables.userId] });
    },
  });
}
...
export function useDeleteTrip() {
  return useMutation({
    mutationFn: async ({ id, userId }: { id: string; userId: string }) => {
      ...
      const collections = ["flights", "hotels", "transport", "restaurants", "activities"];
      await Promise.all(
